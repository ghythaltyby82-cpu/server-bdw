...
const multer = require('multer'); // مكتبة رفع الملفات (الإيصالات)
const helmet = require('helmet');
const rateLimit = require('express-rate-limit');

// --- تفعيل حماية Helmet ---
app.use(helmet({
    contentSecurityPolicy: false, // لضمان عدم تعارض سكربات الأصفار والصور المحلية مع المتصفح
}));

// --- إعداد نظام تحديد المعدل (Rate Limiting) لمنع السبام ---
const limiter = rateLimit({
    windowMs: 15 * 60 * 1000, // 15 دقيقة
    max: 100, // أقصى عدد طلبات مسموح به لكل آيباد خلال المدة
...
// حماية خاصة لصفحات تسجيل الدخول والتقديمات لمنع التكرار السريع
...
// إعداد رفع الملفات (الإيصالات) - مجلد مؤقت لرفع الصور
const upload = multer({ dest: 'uploads/' });

// رابط الويب هوك الخاص بديسكورد سيرفر البادية
...
// --- ملفات لوحة التحكم (الأخبار والمنتجات والتقديمات العامة وتفعيل القوانين) ---
...
const client = new Client({
    intents: [
        ...
        GatewayIntentBits.GuildPresences // مهم جداً لقراءة حالة متصل/غير متصل للأعضاء
    ]
});
...
app.get('/', async (req, res) => {
    try {
        ...
        if (guild) {
            ...
            // حساب عدد المتصلين الحقيقيين (Online, Idle, DND)
...
// --- مسارات إتمام الطلب والدفع الجديدة ---
...
app.listen(3000, () => { console.log('السيرفر يعمل على الرابط : http://localhost:3000'); });